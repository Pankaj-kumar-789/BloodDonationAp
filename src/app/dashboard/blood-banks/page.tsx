      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-gray-100 dark:border-slate-800 shadow-[0_2px_10px_rgba(0,0,0,0.02)] transition-colors">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium text-[15px] transition-colors">Find and contact registered blood banks near you.</p>
          </div>
        ...
        {bloodBanks.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {bloodBanks.map((bank) => (
              <div key={bank.id} className="p-6 border border-gray-100 dark:border-slate-800 rounded-3xl hover:border-red-200 dark:hover:border-red-900/50 hover:shadow-md transition-all group bg-white dark:bg-slate-800/50">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-red-50 dark:bg-red-950/30 flex items-center justify-center shrink-0 border border-red-100/50 dark:border-red-900/30 group-hover:bg-primary-red transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-[13px] font-medium text-gray-500 dark:text-gray-400 mt-0.5 transition-colors">{bank.user.email}</p>
                    </div>
                  </div>
                </div>
                ...
                {bank.user.phone && (
                  <a href={`tel:${bank.user.phone}`} className="mt-4 w-full bg-red-50 dark:bg-red-950/30 hover:bg-red-100 dark:hover:bg-red-900/50 text-primary-red dark:text-red-400 font-bold py-3 px-4 rounded-xl text-[14px] transition-colors flex items-center justify-center gap-2">
          ...
          <div className="text-center py-20 bg-gray-50 dark:bg-slate-800/50 rounded-3xl border border-dashed border-gray-200 dark:border-slate-700 transition-colors">
            <div className="w-20 h-20 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
              <Building2 className="w-10 h-10 text-gray-300 dark:text-gray-600 transition-colors" />
            </div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3 transition-colors">No Blood Banks Found</h3>
