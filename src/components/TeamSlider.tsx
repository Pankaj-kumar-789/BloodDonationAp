...
export default function TeamSlider({ members }: { members: TeamMember[] }) {
  ...
  const safeIndex = Math.min(currentIndex, members.length - 1);
  const currentMember = members[safeIndex];
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="text-center md:text-left flex-1">
            ...
            <h3 className="text-3xl font-black text-gray-900 dark:text-white mb-1">{currentMember?.name}</h3>
            <p className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-4">{currentMember?.role}</p>
            ...
            <div className="flex items-center justify-center md:justify-start gap-4 mt-2">
              ...
              {currentMember?.linkedin && (
                <a href={currentMember.linkedin} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-[#0077b5] text-white flex items-center justify-center hover:scale-110 transition-all shadow-md shadow-blue-200 dark:shadow-none group">
              ...
              {currentMember?.instagram && (
                <a href={currentMember.instagram} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-gradient-to-tr from-[#f09433] via-[#e6683c] to-[#bc1888] text-white flex items-center justify-center hover:scale-110 transition-all shadow-md shadow-pink-200 dark:shadow-none group">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" x2="17.51" y1="6.5" y2="6.5"/></svg>
                </a>
              )}
              {/* Facebook */}
              {currentMember?.facebook && (
                <a href={currentMember.facebook} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-[#1877F2] text-white flex items-center justify-center hover:bg-[#166FE5] hover:scale-110 transition-all shadow-md shadow-blue-200 dark:shadow-none group">
